 ...
 * @author Enaium
 */
object ServerPlayerExecuteCallbackImpl : ServerPlayerCallbacks.ExecuteCallback {
    override fun cancel(player: Player, command: String): Boolean {
        return when (command.split(" ")[0]) {
            "r", "register", "l", "login" -> false
            else -> !Authenticate.isAuthenticated(player)
        }
    }
} 