...
fun register(): AbstractCommand {
    return object : AbstractCommand() {
        ...
        override fun execute(commandSource: CommandSource, args: Array<String>) {
            args.size < 2 && return
            if (commandSource is ServerPlayerEntity) {
                ServerCommandCallbacks.RegisterCallback.EVENT.invoker.execute(
                    Player(
                        commandSource.uuid,
                        commandSource.networkHandler.connection.address
                    ), args[0], args[1]
                ) {
                    commandSource.sendMessage(LiteralText(it))
                }
            }
        }
    ...
    return object : AbstractCommand() {
        ...
        override fun execute(commandSource: CommandSource, args: Array<String>) {
            ...
            if (args[0].equals("ban", ignoreCase = true)
                ...
                MinecraftServer.getServer().playerManager.getPlayer(args[1])?.also { player ->
                    player == commandSource && return
                    ServerCommandCallbacks.BanCallback.EVENT.invoker.execute(
                        ...
                        commandSource.sendMessage(LiteralText(send))
                        disconnect?.also {
        ...
        override fun method_3276(
            ...
            if (strings.size == 2) {
                if (strings[0].equals("ban", ignoreCase = true)) {
                    return MinecraftServer.getServer().playerManager.playerNames.toList()
                }
            }
