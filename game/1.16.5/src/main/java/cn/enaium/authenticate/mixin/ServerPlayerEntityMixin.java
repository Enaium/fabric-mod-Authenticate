...
public class ServerPlayerEntityMixin extends EntityMixin {
    ...
    public boolean isInvulnerable(boolean original) {
        return ServerPlayerCallbacks.InvulnerableCallback.Companion.getEVENT().getInvoker().value(new Player(uuid, networkHandler.getConnection().getAddress()));
    }

    @Override
    public boolean isInvisible(boolean original) {
        return ServerPlayerCallbacks.InvisibleCallback.Companion.getEVENT().getInvoker().value(new Player(uuid, networkHandler.getConnection().getAddress()));
    }
}
